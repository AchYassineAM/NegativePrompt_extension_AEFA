...
# Ajoute la racine du repo (dossier ou se trouve config.py) au PYTHONPATH
# Depuis h2/scripts/ -> parents[2] = racine du projet
ROOT = Path(__file__).resolve().parents[2]
H2_DIR = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(Path(__file__).resolve().parent))  # pour stimuli_paper
...
output_path = H2_DIR / "stimuli_mapping.csv"
