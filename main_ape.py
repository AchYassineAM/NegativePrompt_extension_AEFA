...
from config import PROMPT_SET, Negative_SET, APE_PROMPT_SET, APE_PROMPTs, STIMULUS_GROUPS
import template
import os
import random
import numpy as np
...
def run(
    ...
    print('Stimulus group: ', stimulus_group)
    print('Stimulus index: ', stimulus_index)
    print('Legacy pnum: ', pnum)
    ...
    dir_path = f'results/ape/{result_group}/{model}'
    if os.path.exists(dir_path) == False:
        os.makedirs(dir_path)

    with open(f'{dir_path}/{task}.txt', 'a+', encoding='utf-8') as f:
    ...
    csv_path = f'{dir_path}/results.csv'
